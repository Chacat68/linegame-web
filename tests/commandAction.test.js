...
describe('CommandAction helpers', function () {
  it('会为市场动作生成统一 command 元信息', function () {
    ...
    expect(getCommandKicker(action)).toBe('市场 · 市场情报区');
  });
  ...
  it('会为探索动作生成统一按钮内容与 data 属性', function () {
    ...
    expect(attrs).toContain('data-command-intent="POI 调查"');
    expect(html).toContain('探索 · POI 调查');
  ...
  it('会生成统一的动作反馈文本', function () {
    ...
    expect(text).toBe('📊 已打开当前市场 · 现货交易区。下一步：补足燃料。返回：任务页继续推进当前任务。');
  });
});
