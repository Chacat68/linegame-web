...
- ✅ UI 应用组合所有权：`GameUiApplicationRuntime` 由 433 行降至 430 行，只保留惰性 controller 图与顶层生命周期；`GameUiNavigationPort` 独占冻结导航协议，`GameUiApplicationDiagnostics` 纯组合 Coordinator 渲染/会话、Feature recovery、Context Inspector、Navigation、Workspace Surface、共享 L4、Blocking/Escape 与子控制器 diagnostics
