...
| `MapUI.js` | 重构前 674 行；当前 331 行 | 星系总览、视图状态、星球/POI 投影、探索详情、面板根节点/几何/DOM、动作协议、Context/Escape、Renderer/EventBus/DOM listener 生命周期、导航/旅行/引导聚焦、市场入口和通用 Tab 状态机均已迁出；现只组合星图会话、视图与窄动作端口，并转发公开 API，不直接访问 DOM |
...
| `MapPanelController.js` | 131 行 | 独占详情根节点解析，以及星系、星球、POI、披露区和 Escape 的委派协议；只调用注入动作端口，MapUI 不再访问 document 或解析 dataset intent |
...
| `MapNavigationController.js` | 282 行 | 独占工作区请求、星系/星球切换、同/跨星系旅行分发、引导目标聚焦、Context/面板刷新与 Renderer 对焦；reset 保留 wiring，dispose 清空动作端口 |
...
| `WorkspaceDetailSurface.js` | 387 行 | 已成为五工作区共享的 L4 非阻塞详情层，拥有不可变 detail key、renderer registry、逐层 Escape、焦点恢复、被覆盖 Context 的 inert 处理与冻结投影快照；地图探索档案是首个真实两层 adapter，状态已进入顶层 diagnostics |
...
| `SettingsModalController.js` | 329 行 | 独占设置弹层内部控件、分页键盘、命令反馈、危险确认、Blocking Surface owner token 与完整释放 |
...
| `SaveWorkspaceController.js` | 357 行 | 独占单一存档根事件委托、保存/读取/删除确认、typed command 发布、导入导出、FileReader 失效保护、重绘后槽位焦点恢复、临时资源释放与 diagnostics |
...
| `ContextInspectorViewAdapter.js` | 302 行 | 独占 Inspector 根节点解析、ARIA/内容态、工作区 renderer 宿主、Toggle/Close/Action 委派、空态/壳层写入、焦点恢复与 DOM 完整释放；不持有 Session、renderer registry 或 Escape |
| `ContextInspectorController.js` | 307 行 | 只编排 Session、renderer/action 注册、latest-state 读取、Escape layer、视口模式开合、ViewAdapter 与冻结 diagnostics；不再查询 DOM、绑定 listener、直接聚焦元素或向诊断泄漏领域对象 |
...
| `GameUiApplicationRuntime.js` | 当前 434 行 | 只保留 MarketWorkspace、Settings、Coordinator、Lifecycle 与 Context adapters 的惰性组装，以及 ensure/render/invalidate/reset/dispose 顶层生命周期；不再内联 navigation 与复合 diagnostics |
| `GameUiNavigationPort.js` | 49 行 | 独占领域、命令与引导共享的冻结惰性导航 API；每次调用读取最新 owner，缺失端口安全降级 |
| `GameUiApplicationDiagnostics.js` | 36 行 | 纯组合 Coordinator renderer/session、Feature recovery、Context Inspector、Event UI、Navigation、Workspace Surface、Workspace Detail、SurfaceManager 与子控制器 diagnostics；缺失惰性 controller 时返回稳定可序列化空快照 |
...
| `HudInteractionController.js` | 281 行 | 独占日志事件、胜利弹层 dismiss token、星图工具、Context 初始化、`900px` 断点 listener 与完整 dispose/re-init；dispose 先隐藏胜利弹层再释放 owner，长期路线选择只调用注入 typed action |
| `VictoryProgressPresenter.js` | 147 行 | 纯渲染长期路线摘要与详情，统一领域内容转义、缺口排序与进度可读语义 |
| `Modal.js` / `ActionConfirmUI.js` | 256 / 165 行 | 分别独占交易确认与应用危险确认的稳定 DOM listener、dismiss token、关闭与重建；UI shutdown 显式 dispose，不再用空 callback 维持旧单例 |
| `OnboardingUI.js` | 207 行 | 公司命名在确认/跳过/Feature dispose 后释放，不保留跨流程永久 owner |
| `EventUI.js` | 重构前 343 行；当前 66 行 | 只保留 pending 事件、业务兼容 API 与冻结 diagnostics；DOM 未就绪时保留 pending，旧弹层迟到提交不清除后来事件 |
| `EventPresenter.js` | 108 行 | 无 DOM 地规范化风险/阶段，纯生成并冻结事件标签、摘要、影响预览、fallback 与选择项语义 |
| `EventSurfaceController.js` | 312 行 | 独占事件 Blocking Surface、DOM 构造、方向键、短视口聚焦项可见性、提交防重、listener/dismiss token 释放和可序列化 diagnostics |
| `BlockingSurfaceDismissRegistry.js` | 114 行 | 独占每个 Blocking Surface 的单一 backdrop listener、多 owner 先到先用仲裁、幂等 release 与最后 owner 清理；不认识层级、焦点或工作区 |
| `SurfaceManager.js` | 385 行 | 只编排 Blocking Surface、焦点陷阱、状态观察、dismiss registry、唯一 Escape dispatcher 与空闲释放；只公开可注销注册，diagnostics 不泄漏 L3 workspace DOM/回调 |
...
| 全部 CSS | 29,184 行 | 级联和其他功能的重复响应式规则仍需继续收敛；Header、Bottom Nav、Action Guide、Starmap Controls、Blocking Surface Shell、Market、Settings、Save Workspace 与 Event/Dialogue Flow 均已形成明确 owner，legacy 中 162 个 Header、97 个 Bottom Nav 规则/混合选择器、423 行 Action Guide/Command Slot、1,425 行旧星图控制轨/HUD 小窗、724 行退役 Company Directives UI 及 566 行旧市场面板/重复规则已物理清除；Context Inspector、Workspace Detail、237 行 Blocking Shell、1,341 行 Settings、763 行 Save Workspace 与 784 行 Flow 样式已拆成独立文件，`global-shell-v2.css` 从 789 行降至 72 行；窄屏 L3 Header、空态 Inspector、通用 L6、设置/存档工作区与事件/剧情 flow surface 几何已加入静态/真实浏览器回归保护 |
| `interstellar-trader.css` | 5,534 行 | 仍是主要遗留级联源，但已完全移除 Header、Bottom Nav、旧星图控制轨、隐藏 3D 按钮、五个并行 HUD 小窗、Company Directives UI、全部 Market Feature、Settings、Save Workspace、Event/Dialogue Flow 私有选择器和通用 Blocking Shell 选择器；混合规则只保留其他有效选择器，且不再包含 exploration-terminal/current-system-card/company-name-btn 无 DOM 命中规则 |
...
| `modals.css` | 160 行 | 只保留交易步进器、按钮、滚动条和船体值等仍有运行时消费者的组件规则；通用 Blocking Shell、事件/剧情私有级联与 373 行无入口 Company Directives modal 已迁出或删除 |
...
- `ContextInspectorViewAdapter`：Inspector 根节点/ARIA、renderer 宿主、DOM 事件、空态/壳层与焦点恢复。
- `ContextInspectorController`：renderer/action latest-state、Session、Escape、开合策略与 ViewAdapter 编排。
...
| Settings Workspace CSS owner | **设置弹层私有级联与紧凑信息布局已收口** | 1,341 行 `settings-workspace.css` 独占 launcher、弹层、分页、控件、延迟状态、焦点与响应式布局；`settings/responsive/interstellar-trader/surfaces/bridge-responsive` 中 257 个有效私有选择器分支归零，11 个无消费者旧类删除。1280×720 数据页迁移前后计算样式/几何完全一致；360×640 摘要由 251px 单列改为约 127px 的 2×2 网格，内容区增高且保持单一滚动、零横向溢出、44px 控件、方向键分页和关闭焦点返回；主 CSS 为 283.29 kB / gzip 50.19 kB | Save 子组件继续只在 `save-workspace.css` 中声明带 Save 私有类的宿主规则；其他文件不得回流 `#settings-*` / `.settings-*` |
| Event / Dialogue Flow CSS owner | **L6 信息层级、选择动作与响应式级联已收口** | 784 行 `flow-surfaces.css` 独占 `.event-*`、`.dialogue-*`、事件选择文案、动作区、动效与断点；`modals/responsive/interstellar-trader` 的 165 个原私有选择器分支归零，静态 owner 守卫当前覆盖 175 个分支。1280×720 事件桌面计算样式/几何保持一致；600px、390×844 与 360×640 确认三项摘要完整、影响/分支卡 2×2、单一内容滚动、零横向溢出，事件在内容可容纳时不再空铺满高屏；剧情 choice mode 的伪隐藏 disabled 动作已真正退出布局，剩余 Skip 动作全宽且 44px，焦点环可见，console 无 error/warn；主 CSS 为 286.63 kB / gzip 50.67 kB | 后续事件演出增强只扩展 Presenter/Controller 与该 owner，不得把 Feature 私有选择器回流通用 Modal 或主题覆盖层 |
| Blocking Surface Shell CSS owner | **L6 公共几何、滚动与动作契约已收口** | 237 行 `blocking-surfaces.css` 独占 overlay、低权重 box、结构标题、stack scroll、sticky/fixed actions、安全区、焦点与断点；`modals/responsive/interstellar-trader/surfaces/bridge-responsive` 的 70 个通用选择器分支归零，7 个无 DOM 消费者旧结构类删除。1280×720 交易/确认/派遣宽度保持 540/560/720px；390×844 与 360×640 确认短内容居中、长 stack 单滚动、派遣动作区从视口外回到首屏、44px 动作和零横向溢出；真实 Settings 由 Feature owner 保持外壳 hidden overflow + 正文单滚动，console 无 error/warn；主 CSS 为 284.26 kB / gzip 50.29 kB | Feature 只设置 `--modal-w` 与内容布局，不得复制公共 overlay/viewport/action shell；静态守卫拒绝通用选择器、共享变量和退役类回流 |
| Save Workspace Presenter/Controller + `SaveUI` + CSS owner | **存档工具 UI 所有权、单一命令端口与移动验收已接入** | Presenter 纯投影安全/槽位/迁移/确认描述；Controller 独占单一根委托、确认、JSON 文件选择、Blob 下载、异步读取失效、删除/导入重绘后同槽位焦点恢复和释放，并通过 Adapter 发布保存/读取 typed command；Feature manifest、GameUiCoordinator reset 与顶层 diagnostics 已接线；`SaveUI` 由 447 行降至 18 行请求对象门面；763 行 `save-workspace.css` 独占全部私有选择器，五个旧 owner 私有命中归零；1280×720、390×844 迁移前后计算样式/几何指标一致，360×640 确认单一内部滚动、零横向溢出、44px 控件、删除确认接管与 nearest 焦点回显，console 无 error/warn；主 CSS 构建为 277.75 kB / gzip 49.56 kB | 未来云同步必须作为独立 effect/port，不得回流门面；真实文件选择仍遵循浏览器权限，不以自动化注入用户文件 |
...
| `OnboardingUiController` + `OnboardingPolicyController` | **首次进入 UI 生命周期与内容策略已接入** | UI controller 持有教程视图同步、开始/跳过决策、公司身份入口 DOM listener 与 latest-session 校验；policy 持有欢迎消息、教程完成反馈和首批任务推荐；入口 listener 由 UI runtime shutdown 释放，Onboarding Feature dispose 同时关闭公司/教程弹层并释放 dismiss owner | 把可配置文案迁入内容资源层 |
...
| `GameUiLifecycleController` | **eager UI 壳 bind / present / dispose 已接入** | HUD、MapUI、UIManager、Modal、Action Guide、设置/公司 launcher、Feature telemetry、场景就绪和首次进入呈现统一接线；重复初始化只保留一个教程完成 listener；shutdown 显式 dispose 交易 Modal 与 ActionConfirmUI，并释放 HUD/MapUI/UIManager 的 DOM、EventBus、Context、Escape 和全局 facade/callback | 补浏览器级 listener 计数 |
| `GameUiApplicationRuntime` + `GameUiNavigationPort` + `GameUiApplicationDiagnostics` | **UI 应用组合、导航与诊断 owner 已拆分接入** | 434 行 Runtime 只保留 MarketWorkspace、Settings command/UI、GameUiCoordinator、GameUiLifecycle 与 Context adapters 的惰性组装及 ensure/render/invalidate/reset/dispose；49 行 NavigationPort 独占正式冻结导航协议；36 行 ApplicationDiagnostics 纯组合 Coordinator 渲染/会话、Feature recovery、Context Inspector、Event UI、Navigation、Workspace Surface、Workspace Detail、SurfaceManager 与子控制器快照，惰性 owner 缺失时保持稳定可序列化降级；`settingsCommands.execute` 继续作为正式端口公开 | 继续收窄剩余 UI typed ports，并把 feature fallback 统一到同一状态呈现 |
...
| `MapPanelViewController` + `MapPanelController` + `MapContextController` + `MapInteractionController` + `MapNavigationController` | **星图面板、Context、交互生命周期与导航用例 owner 已接入** | 面板 DOM/ARIA/滚动/几何、根节点/委派协议、Context key/Renderer selection/Escape、Renderer 全局回调/EventBus/DOM listener、工作区请求/视图切换/旅行分发/引导聚焦分别只有一个 owner；直接测试覆盖 latest-state、访问校验、本地旅行/跨星系跳转、工作区/Context/Renderer 原子聚焦、根节点绑定、reset/dispose 与 re-init，`MapUI` 从 674 行降至 331 行无 DOM 组合门面 | 后续新增地图交互只能扩展窄端口，不得把导航解释、DOM、Context 注册或全局回调写回门面 |
| `WorkspaceTabController` | **Archive/Fleet Tab owner 与顶层 diagnostics 已接入** | 独占 listener、roving tabindex、`aria-selected/aria-hidden`、方向键、移动端 scroll-into-view、程序化深链、关闭/背景 dismiss 与 dispose；GameUiLifecycle 直接初始化，command/guidance 经正式 UI runtime navigation port 调用；活动 Archive/Fleet Tab、激活/监听计数与生命周期已作为 `workspaceTabs` 进入 UI application 顶层快照；MapUI 不再导出 Tab facade | 后续 Tab 状态只能由 Controller diagnostics 扩展，不得在门面或 Runtime 复制 |
...
| `NavigationController` | **已接入 `UIManager` 并汇入顶层 diagnostics** | 五个 workspace、旧别名、唯一 active、幂等切换、独立冻结 detail stack；MapUI 通过显式 navigation port 请求切换，不再持有底栏 listener 或全局 manager facade；Escape 只关闭 L4 详情不改变 L3；会话 reset 一次清空当前与隐藏 workspace 的全部 detail stack，但保留 L3 目的地；canonical active/L4 栈与深度由 UI application 顶层快照直接透传 | 后续导航诊断只能扩展纯快照，不得缓存 state 或建立第二事实源 |
| `WorkspaceSurfaceController` | **五个同级 L3 已接入并汇入顶层 diagnostics** | map/trade/fleet/archive/logs 均为 `#game-main` 直属 `.workspace-surface`；市场不再嵌入星图，所有工作区共用 `is-active/data-workspace-active/inert/aria-hidden`、唯一可见诊断与来源相关焦点；底栏保留 nav 焦点，程序化进入在 Feature 完成后提交标题焦点并丢弃迟到结果；可见 Surface ID、一致性、激活与焦点计数由 UI application 顶层快照直接透传；无 DOM 命中的旧规则与有效根 ID 的跨层几何覆盖均已删除，PostCSS 契约阻止回流 | 把更多工作区内部选中态留在各 Feature session diagnostics，不在 Surface owner 复制 |
| `SurfaceManager` | **Blocking / Escape owner 与顶层 diagnostics 已接入** | blocking 层优先且不下穿；非阻塞层按优先级处理；primary/secondary workspace API、状态和焦点映射已删除，模块不再认识 market/info/trade/console DOM；可见 Blocking ID、顶层 Surface、dismisser/observer/focus-return 计数及注册/活动 Escape layer 优先序以冻结可序列化快照进入 UI application diagnostics，不泄漏 DOM/回调 | 继续收束各 modal 的声明式 metadata，并为 controller dispose 增加绑定释放协议 |
| `ContextInspectorSession` + `ContextInspectorPresenter` + `ContextInspectorViewAdapter` + `ContextInspectorController` | **五个对象型 workspace 已接入并完成职责拆分** | Session 独占每 workspace 的不可变 context key、常规/紧凑视口独立开合偏好与 revision；Presenter 纯生成壳层/空态；302 行 ViewAdapter 独占 DOM/ARIA/宿主/事件/焦点；307 行 Controller 只编排 latest-state renderer/action、Session、Escape、响应式开合与冻结 diagnostics；490 行旧模块收束为 36 行门面。地图、商品、舰船、任务、科技、派系、成就、探索报告与只读日志消息均已接入，archive 分类切换会清理旧 Context；Context/renderer workspace、计数、开合和 ViewAdapter 生命周期已进入 UI application 顶层快照且不包含领域对象/DOM；HUD 的断点 listener 会在运行中切换会话模式并随 dispose 释放 | 继续减少完整终端内的重复详情，并用统一 diagnostics 支撑浏览器验收定位 |
| `WorkspaceDetailSurface` | **统一 L4 与顶层 diagnostics 已接入 map + trade + fleet + archive + logs** | 从 Navigation detail stack 投影单一非阻塞详情面；按 type 注册 renderer，使用 latest-state/revision；陈旧详情自动退出；Escape 只退一层并精确恢复焦点；地图探索已有两层真实链路，商品、舰船、五类档案对象与只读日志消息均可进入独立 L4；日志淘汰会刷新并关闭失效详情；当前 workspace/depth/open、renderer type 与开合/渲染计数已进入 UI application 顶层快照 | 继续减少完整终端中的重复详情并细分工作区局部刷新 |
