...
| `MarketUI.js` | 重构前 3,606 行；当前 1,358 行 | 图表、现货、商品、商品详情、资金和贸易站投影边界已迁出；现在主要持有工作区/成长状态、增量重绘、价格总览、局部选择和 typed command 发布 |
...
| `GameUiCoordinator` + `ActionPresentation` | **dirty-region 增量刷新、Fleet / Archive 内部区域与 Feature 恢复已接入真实动作** | provider、四项 Feature ensure/render、命名 action 分组；市场专用刷新与通用 ensure 共用 `loadFeature`，失败发布局部重试，成功清理状态并按 latest-state 渲染；Fleet 动作已区分 `fleet-hangar` / `fleet-shop`，Archive 五页也有独立区域且 Quest/Research 真实动作与对话后续刷新已接入；活动工作区只刷新声明 presenter，隐藏工作区不后台重绘，显式整体失效优先且去重 | 消除 action pipeline 的两个兼容全量 fallback，并拆出 MarketUI 的真实内部 render 端口 |
