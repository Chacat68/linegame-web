...
| `GuidanceExecutionAdapter` | **行动引导执行端口已接入** | actionType 执行上下文改由分组端口组装；异步 Feature 加载使用 latest-session 校验，迟到结果被丢弃；不可用与执行异常统一反馈并恢复 Guide | 将教学链启动与剩余 direct-execution policy 迁出组合根 |
