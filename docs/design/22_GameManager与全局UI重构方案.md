...
| `MapUI.js` | 1,055 行 | 星系总览、视图状态、星球/POI 投影、探索详情、面板几何计算、动作协议和可丢弃会话状态均已迁出；现只持有 Context、Renderer、DOM 测量/样式投影、市场入口和视图协调；L3 只经显式 navigation port 请求 |
...
| `MapExplorationPresenter` + `MapPlanetDetailPresenter` + `MapPanelLayout` + `MapPanelController` | **星球/POI 投影、布局和动作协议已接入** | POI 阻塞/完成流程、秘密航线、勘探入口、星球摘要、航线焦点、路线估算、档案披露、travel intent、浮动面板几何和统一委派均已迁出 `MapUI`；旧探索终端 Runtime/EventBus/Session/CSS 全链删除 | 将市场入口和历史 tab 兼容职责继续迁出 `MapUI` |
