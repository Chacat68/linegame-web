...
| 全部 CSS | 28,773 行 | 级联和其他功能的重复响应式规则仍需继续收敛；Header、Bottom Nav、Action Guide、Starmap Controls、Market 与 Save Workspace 均已形成明确 owner，legacy 中 162 个 Header、97 个 Bottom Nav 规则/混合选择器、423 行 Action Guide/Command Slot、1,425 行旧星图控制轨/HUD 小窗、724 行退役 Company Directives UI 及 566 行旧市场面板/重复规则已物理清除；Context Inspector、Workspace Detail 与 763 行 Save Workspace 样式已拆成独立文件，`global-shell-v2.css` 从 789 行降至 72 行；窄屏 L3 Header、空态 Inspector、存档工作区与事件 flow surface 几何已加入静态/真实浏览器回归保护 |
| `interstellar-trader.css` | 6,645 行 | 仍是主要遗留级联源，但已完全移除 Header、Bottom Nav、旧星图控制轨、隐藏 3D 按钮、五个并行 HUD 小窗、Company Directives UI、全部 Market Feature 与 Save Workspace 私有选择器；事件/剧情 flow Grid 使用不可压缩内容行和单一内部滚动，混合规则保留其他有效选择器，且不再包含 exploration-terminal/current-system-card/company-name-btn 无 DOM 命中规则 |
