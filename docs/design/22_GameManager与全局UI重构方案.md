...
| 全部 CSS | 31,217 行 | 级联和其他功能的重复响应式规则仍需继续收敛；Header、Bottom Nav 与 Action Guide 组件均已形成明确 owner，legacy 中 162 个 Header、97 个 Bottom Nav 规则/混合选择器及 423 行 Action Guide/Command Slot CSS 已物理清除；Context Inspector 与 Workspace Detail 已拆成独立文件，`global-shell-v2.css` 从 789 行降至 129 行 |
