...
- Context Inspector 所有权已拆为 `ContextInspectorSession`（不可变 key/工作区开合/revision，并隔离常规/紧凑视口偏好）、`ContextInspectorPresenter`（壳层/空态）、`ContextInspectorViewAdapter`（DOM/ARIA/工作区宿主/事件/焦点）与 `ContextInspectorController`（latest-state renderer/action/Session/Escape/响应式开合），Controller 从 426 行降至 283 行，原模块保持 35 行兼容门面
