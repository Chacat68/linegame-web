...
- UI 应用组合边界已拆为 430 行 `GameUiApplicationRuntime`、49 行 `GameUiNavigationPort` 与 35 行 `GameUiApplicationDiagnostics`，惰性 controller 图/生命周期、正式导航协议和含 Context Inspector、Navigation、Workspace Surface、共享 L4、Blocking/Escape 状态的顶层只读快照分别只有一个 owner
