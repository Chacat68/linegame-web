...
function _findQuestForContext(state, questId) {
...
export function renderContextInspector(request) {
  ...
  var match = _findQuestForContext(state, context.id);
...
export function renderWorkspaceDetail(request) {
  ...
  var state = request && request.state;
  var container = request && request.container;
  if (!detail || detail.type !== 'archive-quest' || !state || !container) return false;
  var match = _findQuestForContext(state, detail.id);
  if (!match) return false;
  var quest = match.quest;
  var type = QUEST_TYPES[quest.type] || { icon: '📋', name: '任务' };
  var reward = Quest.getQuestRewardSummary(state, quest);
  ...
  var facts = objectives.map(function (objective, index) {
    ...
    return {
      ...
      value: _objectivePlanText(objective),
